...
see Design notes), a compressed markdown file `kb/videos/<id>.md`, a
regenerated `kb/index.md`, and an updated record in the central log registry
`log/videos.json`.
...
├── transcripts/                    # gitignored - local audit trail only
...
└── kb/
    ├── videos/<id>.md              # compressed notes, ONE FILE PER VIDEO
    └── index.md                    # generated: title + chapter headings per video
```

`kb/` is one file per video rather than one accumulating file because the
reader skill (`tokovinin-kb-context`) pulls it over HTTP: the whole base is
~124k tokens, while `index.md` plus the one or two videos a question needs
is ~13k. `index.md` is generated by `scripts/kb_index.py` - never hand-edit it.

`transcripts/` is gitignored: 50 MB of raw auto-captions for 64 videos, which
nothing downstream reads once `_full.txt` has been compressed, and which would
otherwise make every clone of this public repo carry them.
...
before, just without a precise per-line boundary).

Write it as a **new file** `kb/videos/<id>.md` — one video per file, never
appended to anything. The file starts with the `## <title>` line and the
`id: <id> | <duration> | <url>` line (that exact shape is what `kb_index.py`
parses), then `Тема:`, then the `### <chapter> (MM:SS–MM:SS)` sections.
Never rewrite other videos' files unless the user asks.

Then regenerate the index — the reader skill can't find a video that isn't
in it:
```bash
python3 SKILL_DIR/scripts/kb_index.py --kb-dir kb
```

Measure the token counts (raw `_full.txt` vs. the new `kb/videos/<id>.md`) so
the notification can report the compression ratio, then mark the stage done
and send Notification 2/3:
```bash
python3 -c "import tiktoken; e=tiktoken.get_encoding('cl100k_base'); print(len(e.encode(open('transcripts/<id>_full.txt').read())), len(e.encode(open('kb/videos/<id>.md').read())))"
...
clobbering `kb/` / `log/videos.json` if they already have real content.
